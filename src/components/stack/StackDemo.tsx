...
export default function StackDemo() {
  ...
  return (
    ...
    >
      ...
      <div
        ref={gridRef}
        className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 divide-x divide-y divide-[#1e1e1e] border-b border-[#1e1e1e]"
      >
        {STACK_DATA.map((section) => (
          <div
            key={section.id}
            className="group flex flex-col p-4 sm:p-6 md:p-8 hover:bg-[#0d0d0d] transition-all duration-300 relative overflow-hidden"
          >
            ...
            <div className="flex items-start justify-between mb-4 sm:mb-6">
              <span
                className="text-[28px] sm:text-[36px] md:text-[40px] leading-none text-[#1e1e1e] group-hover:text-[#2a2a2a] transition-colors duration-300 tabular-nums"
                style={{ fontFamily: "'Bebas Neue', sans-serif" }}
              >
                {section.num}
              </span>
              <span
                className="text-[8px] sm:text-[10px] font-mono tracking-[1px] sm:tracking-[1.5px] uppercase border px-1.5 sm:px-2 py-0.5"
            ...
            {/* Tagline */}
            <p className="text-[11px] sm:text-[12px] text-[#888] group-hover:text-[#f0ede6] font-mono leading-[1.7] mb-4 sm:mb-6 transition-colors duration-250">
              {section.tagline}
            </p>

            {/* Tech tags */}
            <div>
              <p className="text-[8px] sm:text-[9px] font-mono tracking-[1.5px] text-[#333] uppercase mb-2 sm:mb-3">
                Core Technologies
              </p>
              <div className="flex flex-wrap gap-1 sm:gap-1.5">
                {section.items.slice(0, 6).map((item) => (
                  <span
                    key={item.name}
                    className="text-[8px] sm:text-[9px] px-1.5 sm:px-2 py-0.5 border border-[#1e1e1e] text-[#555] group-hover:text-[#f0ede6] group-hover:border-[#2a2a2a] font-mono tracking-[0.5px] sm:tracking-[1px] transition-all duration-200"
                  >
                    {item.name}
                  </span>
                ))}
                {section.items.length > 6 && (
                  <span className="text-[8px] sm:text-[9px] px-1.5 sm:px-2 py-0.5 text-[#333] font-mono">
